...
/**
 * HTML tags which are used inline, where whitespace must be respected
 */
PrintXml.prototype.inlineTags = [
  "b", "big", "i", "small", "tt",
  "abbr", "accronym", "cite", "code", "dfn", "em", "kbd", "strong", "samp", "var",
  "a", "bdo", "br", "img", "map", "object", "q", "script", "span", "sub", "sup",
  "button", "input", "label", "select", "textarea"
];

/**
 * DocType declaration to output after the <?xml ... ?> declaration
 */
PrintXml.prototype.doctype = null;
...
PrintXml.prototype.printText = function (text) {
  ...
  if (text.indexOf("\n") >= 0) {
    ...
    // this.print(
    //   text.split("\n")
    //     .map(function(line) { return this.printIndent(this.elementNames.length) + line; }, this)
    //     .join("\n"));
...
PrintXml.prototype._transform = function (event, encoding, done) {
  ...
  if (Buffer.isBuffer(event)) {
    ...
    case "start":
      ...
      if (this.hadChildren[0] && this.inlineTags.indexOf(name) == -1) {
    ...
    case "end":
      ...
      if (!autoClose || !this.stillOpen) {
        this.print("</"+name+">");
        if (this.inlineTags.indexOf(name) == -1) {
          this.print("\n");
        }
      }
      this.stillOpen = false;
    ...
    case "print-xml":
      ...
      default:
        this.printComment("Textree invalid 'print-xml' event: "+JSON.stringify(event));
      }
      break;
...
PrintXml.prototype.ensureCloseTag = function(inline) {
  if (this.notYetWritten) {
    if (this.doctype) {
      this.print('<!DOCTYPE '+this.doctype+'>\n');
    } else {
      this.print('<?xml version="1.0" encoding="UTF-8"?>\n');
    }
    this.notYetWritten = false;
  }
  if (this.stillOpen) {
    if (inline) {
      this.print(">");
