...
/**
 * Declaring period properties
 */
Accounting.prototype.command_period = function(event) {
  this.captureLevelEvents(true)
    .then(function() {
      if (event.attributes.start) {
        this.periodStart = new Date(event.attributes.start+" 00:00:00");
      }
      if (event.attributes.end) {
        this.periodEnd = new Date(event.attributes.end+" 23:59:59");
      }
    }.bind(this))
    .done();
};
  ...
  this.captureLevelEvents(true)
    .then(function() {
      ...
      if (account.path) {
        ...
        if (/\/$/.test(account.path)) {
          this.accountsAgg[account.path] = account;
        } else {
  ...
  this.captureLevelEvents(true)
    .then(function() {
      ...
      if (sheetName) {
        ...
        ['sheet', 'code', 'update_type', 'update_source', 'comment', 'date', 'label', 'doc', 'join_sheet',
  ...
  this.captureLevelEvents(true).then(function() {
    ...
    sheet.replacements.forEach(function(replacement) {
      ...
      if (/^[<>]/.test(replacement.match)) {
        ...
        result = compare(row[replacement.column], val);
      } else {
        // expr = new RegExp(replacement.match, 'i');
        result = (new RegExp(replacement.match, 'i')).exec(row[replacement.column]);
      }
      // var result = expr.exec(row[replacement.column]);
      if (result) {
        ...
        row[replacement.target_column] =
          this.templateReplace(replacement.target_value, row, sheet, typeof result == "object" ? result : null);
      }
      if (parseInt(replacement.debug) > 0 &&
          (result || parseInt(replacement.debug) < 2)) {

        this.warn("DEBUG sheet '"+sheet.name+"' on row n°"+sheet.nthRow+": "+JSON.stringify(row)+
                  "\nreplacement = "+JSON.stringify(replacement)+", result = "+
                  JSON.stringify(result)+"\n  VALUE["+replacement.target_column+"]="+row[replacement.target_column]);
    ...
    if (this.periodStart && this.periodStart > date ||
        this.periodEnd && this.periodEnd < date) {
      var period = [this.periodStart ? this.periodStart.toISOString().substring(0, 10) : "",
                    this.periodEnd ? this.periodEnd.toISOString().substring(0, 10) : ""].join('–');

      this.notice("date does not belong to ["+period+"] period (ignored!) in sheet '"+sheet.name+
                  "' on row n°"+sheet.nthRow+": "+JSON.stringify(row));
      return;
    }
    ...
    if (sheet.join_sheet) {
      ...
      for (var i = this.entries.length - 1; i >= 0; --i) {
        ...
        // this.warn("sheet", _entry.sheet.name, sheet.join_sheet, sheet.join_target_match, JSON.stringify(_entry.source));
        // if (_entry.sheet.name == sheet.join_sheet) {
        //   this.warn('MATCH', label, '--', this.templateReplace(sheet.join_target_match, _entry.source,
        //                                                        this.sheets[_entry.sheet.name]));
        // }
        if (_entry.sheet.name == sheet.join_sheet) {
          c++;
        }
      ...
      if (!entry) {
        this.warn("row n°"+sheet.nthRow+" on sheet '"+sheet.name+
                  ...
                  "' (match value: '"+label+"', not found in "+c+" entries)");
        return;
      }
    } else { // sheet.join_sheet

      entry = { sheet: sheet, row: sheet.nthRow,
                date: date, id: sheet.code + ("00000" + sheet.nthRow).slice(-6),
                label: label, doc: "",
...
Accounting.prototype.isZero = function(value) {
  return value.toFixed(2) == "0.00";
};

Accounting.prototype.isPositive = function(value) {
  return value.toFixed(2) != "0.00" && value > 0;
};

Accounting.prototype.isNegative = function(value) {
  return value.toFixed(2) != "0.00" && value < 0;
};

Accounting.prototype.formatDate = function(date, mode) {
  var dateStr = date.toISOString().substr(0, 10);
  switch (mode) {
  case "fr":
    dateStr = dateStr.replace(/^(....)-(..)-(..)/, "$3/$2/$1");
    break;
  case "compact":
    dateStr = dateStr.replace(/^(....)-(..)-(..)/, "$1$2$3");
    break;
  }
...
Accounting.prototype.formatValue = function(value, type) {
  ...
  value = value.toFixed(type == "integer" ? 0 : 2);

  for (var i = value.length - (type == "integer" ? 3 : 6); i > 0; i -= 3) {
...
Accounting.prototype.templateReplace = function(template, row, sheet, numberParams) {
  return (template || "")
    .replace(/{([^}]+)}/g, function(match, columnSpec) {
      ...
      try {
        ...
        if (parts[0][0] == '$') {
          if (numberParams && numberParams[parseInt(parts[0].substr(1))]) {
            value = numberParams[parseInt(parts[0].substr(1))];
            // this.warn("NUMBER "+columnSpec+" ("+parseInt(parts[0].substr(1))+") numberParamslen="+
            //           numberParams.length+" replaced with : "+value+", array: "+JSON.stringify(numberParams));
          } else {
            // this.warn("NUMBER "+columnSpec+" not found ("+parseInt(parts[0].substr(1))+") numberParamslen="+
            //           numberParams.length+" for template: "+template);
            return columnSpec;;
          }
        } else if (!(parts[0] in row)) {
...
Accounting.prototype.getAccounts = function(parentAccount, aggregate, nogenerate) {
  ...
  Object.keys(this.accounts)
    .forEach(function(path, idx, accountsArr) {

      if (!parentAccount || parentAccount.matchAccount(path)) {
        var pathArr = path.split("/");
        var aggPath;

        if (aggregate === undefined || aggregate == "before") {
          for (var i = 0; i < pathArr.length && i < lastPathArr.length && pathArr[i]==lastPathArr[i]; i++)
            ;;
          for (var i2 = i + 1; i2 < pathArr.length; i2++) {
            aggPath = pathArr.slice(0, i2).join("/")+"/";

            if (!nogenerate || this.accountsAgg[aggPath]) {
              accounts.push(Object.assign({
                isAggregate: true,
                path: aggPath,
                commonPath: i2 > 1 ? pathArr.slice(0, i2 - 1).join("/")+"/" : "",
                relativePath: pathArr[i2 - 1]+"/",
              }, this.accountsAgg[aggPath]));
            }
          }
        }
        ...
        if (aggregate && aggregate == "after") {
          var nextAccount = idx == accountsArr.length - 1 ? "" : accountsArr[idx + 1];
          var nextAccountArr = nextAccount.split("/");
          for (var j = 0; j < pathArr.length && j < nextAccountArr.length && pathArr[j]==nextAccountArr[j]; j++)
            ;;
          // console.log("path", path, " <-> ", nextAccount, j);
          for (var j2 = pathArr.length - 2; j2 >= j; j2--) {
            aggPath = pathArr.slice(0, j2 + 1).join("/")+"/";

            if (!nogenerate || this.accountsAgg[aggPath]) {
              accounts.push(Object.assign({
                isAggregate: true,
                path: aggPath,
                commonPath: j2 > 0 ? pathArr.slice(0, j2).join("/")+"/" : "",
                relativePath: pathArr[j2]+"/",
              }, this.accountsAgg[aggPath]));
            }
          }
        }
...
Accounting.prototype.getAccountNumber = function(path) {
  return this.accounts[path] && this.accounts[path].number;
};
Accounting.prototype.getAccountDescription = function(path) {
  return this.accounts[path] && this.accounts[path].description;
};
...
Accounting.prototype.getSheet = function(sheet) {
...
Accounting.prototype.sealEntries = function() {
  if (!this.sealedEntries) {
    this.entries.sort(
      function(a, b) {
        if (!a.date.toGMTString) {
          this.warn("accounting: not a date ("+a.date+") for sheet '"+a.sheet.name+
                    "', row n°"+a.row);
        }
        if (!b.date.toGMTString) {
          this.warn("accounting: not a date ("+b.date+") for sheet '"+b.sheet.name+
                    "', row n°"+b.row);
        }
    ...
    for (var i = 0; i < this.entries.length; i++) {
      ...
      if (!this.isAmountNull(balance)) {
        this.warn("accounting: balance is not zero ("+balance+"), affecting to "+
                  this.waitAccount+" for sheet '"+entry.sheet.name+
                  "', row n°"+entry.row);
