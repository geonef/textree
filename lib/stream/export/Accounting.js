...
Accounting.prototype.command_sheet_row = function(event) {
  ...
  this.captureLevelEvents(true).then(function() {
    ...
    if (this.periodStart && this.periodStart > date ||
      ...
      this.warn("date does not belong to ["+period+"] period (ignored!) in sheet '"+sheet.name+
                "' on row n°"+sheet.nthRow+": "+JSON.stringify(row));
      return;
    }
...
Accounting.prototype.templateReplace = function(template, row, sheet, match) {
  return (template || "")
    .replace(/{([^}]+)}/g, function(match, columnSpec) {
      ...
      try {
        ...
        if (parts[0][0] == '$') {
          if (match && match[parseInt(parts[0].substr(1))]) {
            value = match[parseInt(parts[0].substr(1))];
          } else {
            return columnSpec;;
          }
    ...
    this.entries.sort(
      function(a, b) {
        if (!a.date.toGMTString) {
          this.warn("accounting: not a date ("+a.date+") for sheet '"+a.sheet+
                    "', row n°"+a.row+": "+JSON.stringify(a));
        }
    ...
    for (var i = 0; i < this.entries.length; i++) {
      ...
      if (!this.isAmountNull(balance)) {
        this.warn("accounting: balance is not zero ("+balance+"), affecting to "+
                  this.waitAccount+" for sheet '"+entry.sheet+
