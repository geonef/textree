...
Accounting.prototype.command_sheet_column = function(event) {
  ...
  this.captureLevelEvents(true)
    .then(function() {
      ...
      if (column.sheet) {
        if (!column.column && column.type != 'label') {
          return this.warn("No column defined for sheet '"+column.sheet+"' in meta sheet");
        }
        if (column.column[0] == '?') {
          column.balanced = true;
          column.column = column.column.substring(1);
        }
        if (column.type == "value" &&
            (!column.account || !/^[-+*]/.test(column.account))) {
          return this.warn("Invalid account spec ('"+column.account+"') for sheet '"+column.sheet+
                           "' in meta sheet, column '"+column.column+
                           "' (should start with '+', '-', or '*'");
        }
        if (!this.sheets[column.sheet]) {
          this.sheets[column.sheet] = {
            name: column.sheet, columns: {}, //labelColumns: [],
            nthRow: 1, replacements: [] };
        }

        if (column.type == 'label') {
          this.sheets[column.sheet].label = column.template;
          // this.sheets[column.sheet].labelColumns[column.column] = column;
        } else {
...
Accounting.prototype.command_sheet_row = function(event) {
  ...
  this.captureLevelEvents(true).then(function() {
    ...
    if (sheet.label) {
      entry.label = this.templateReplace(sheet.label, row, sheet);
    }
      ...
      case 'value':
        ...
        if (account) {
          if (!this.isAccountValid(account, true)) {
            ...
            if (value == "") {
              // if (name == 'REST') {
              //   console.log("*** REST", name, column.balanced);
              // }
              if (!column.balanced) { return; }
...
Accounting.prototype.templateReplace = function(template, row, sheet) {
  return template.replace(/{([^}]+)}/g, function(match, column) {
    if (!(column in row)) {
      this.warn("column "+column+" not defined for label template of sheet '"+sheet.name+
                "', on row n°"+sheet.nthRow+": "+JSON.stringify(row));
      return "";
    }
    return row[column];
