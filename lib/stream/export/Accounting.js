...
Accounting.prototype.command_sheet_row = function(event) {
  ...
  this.captureLevelEvents(true).then(function() {
    ...
    // if (sheet.name=="Beds24_facturation" && sheet.nthRow == 3)
      // this.warn("ZZZZZZZZZZZZZ", !!dateStr, "--", dateStr, "--", sheet.date);
    if (!dateStr) {
      return; // ignore rows with no date (even if join)
    }
    ...
    sheet.accounts.forEach(function(accountSpec) {
      ...
      if (account) {
        ...
        if (/^[-+]? *%? *$/.test(amountStr)) { // empty value or percentage
          ...
          // this.warn("empty becomes", account, if_empty);
          if (if_empty) {
            amountStr = if_empty;
            // sign = null;
            // if (/^[-+]/.test(amountStr)) {
    ...
    .replace(/{([^}]+)}/g, function(match, columnSpec) {
      ...
      try {
        ...
        parts.forEach(function(part) {
          ...
          case 'number':
            if (value) { value = ""+this.parseValue(value, row, sheet); }
            break;
