...
Accounting.prototype.getAccountPeriodicStats = function(account/*, balances*/) {
  ...
  var stats = { isAggregate: account.isAggregate, subAccounts: [], periods: [],
                monthly: {}, global: {}, accountsPositive: 0, accountsNegative: 0 };
  // var suite = [];
  // suite.groups = [];
  // suite.balances = {};
  // suite.isAggregate = account.isAggregate;
  var value;
  ...
  this.entries
    ...
    .forEach(function(entry) {
      ...
      if (!value || value.period != period) {
        ...
        var balance = value ? value._balance : 0;
        value = { //period: period,
          _balance: balance,
          _global: 0 };
        ...
        if (account.matchAccount(cur)) {
          if (account.isAggregate) {
            ...
            if (stats.subAccounts.indexOf(childAccount) == -1) {
              ...
              // acc[childAccount] = { balance: 0, debits: 0, credits: 0 };
            }
            //
            if (!value[childAccount]) {
              value[childAccount] = { amount: 0/*, credits: 0, debits: 0*/ };
            }
            value[childAccount].amount += entry.assignment[cur];
            // if (entry.assignment[cur] > 0) {
            //   value[childAccount].credits += entry.assignment[cur];
            // } else {
            //   value[childAccount].debits += -entry.assignment[cur];
            // }
            ///////
            // if (undefined == value[childAccount]) {
            //   value[childAccount] = 0;
            // }
            // value[childAccount] += entry.assignment[cur];
          }
  ...
  for (var date = new Date(earliestDate.valueOf()); date <= latestDate; date.setMonth(date.getMonth() + 1)) {
    ...
    if (!stats.monthly[period]) { stats.monthly[period] = {}; } // This should NOT be needed :(
    if (account.isAggregate) {
      ...
      stats.global.balance += stats.monthly._global;
    }
  }
  stats.subAccounts.forEach(function(account) {
    if (stats.global[account] < 0) { stats.accountsNegative++; } else { stats.accountsPositive++; }
  });
