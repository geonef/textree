...
ProcessNodes.prototype.command_process = function (event) {
  ...
  if (event.attributes["class"]) {
    this.captureLevelAndTransform(event.attributes["class"], event.attributes.id);
 ...
 * Used by command_process().
 */
ProcessNodes.prototype.captureLevelAndTransform = function (_ClassOrObject, bindName) {
  var def = Q.defer();
