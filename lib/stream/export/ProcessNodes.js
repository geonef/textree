...
ProcessNodes.prototype.captureLevelAndTransform = function (_ClassOrObject, bindName, cache) {
  ...
  var isFromCache = bindName && cache && js_cache[cache];
  // if (bindName = cache) {
  //   if (js_cache[cache]) {
  ...
  this.captureLevelEvents(/*false*/ isFromCache, def) // PROBLEM: this is never resolved!
    .then(function(events) {
      if (typeof _ClassOrObject == "string") {
        _ClassOrObject = this.loadModule(_ClassOrObject);
      }
      if (_ClassOrObject) {
        ...
        if (isFromCache) {
          if (stream.deserialize) {
            stream.deserialize(js_cache[cache]);
          } else {
            this.warn("stream has no deserialize method");
          }
        } else {
          ...
          this.sourceReadable(stream)
            .then(function() {
              ...
              if (bindName && cache) {
                if (stream.serialize) {
                  js_cache[cache] = stream.serialize();
                } else {
                  this.warn("stream has no serialize method");
                }
              }
              def.resolve();
            }.bind(this))
