...
Chronology.prototype.addEvent = function(event) {
  if (!event.date) {
    this.warn("missing attribute 'date' for event");
    return;
  }
  ...
  this.sealedEvents = false;
};

/**
 * Delete the events matching a tag and/or property values
 *
 ...
 * @return {number} count       number of deleted events
 */
Chronology.prototype.removeEvents = function(tag, match) {
  ...
  for (var i = this.events.length - 1; i >= 0; i--) {
    ...
    if (match &&
        !Object.keys(match).every(function(matchKey) {
  ...
  this.warn("removeEvents: deleted "+count+" of tag="+tag+" match="+JSON.stringify(match));
  return count;
};
...
// UNUSED YET
Chronology.prototype.findInventory = function(inventory, match) {
  if (!this.inventory[inventory]) {
    throw new Error("invalid inventory '"+inventory+"', inventories are: "+Object.keys(this.iventory).join(", "));
  }
  return this.inventory[inventory].filter(function(item) {
    if (match) {
      return Object.keys(match).every(function(matchKey) {
        return item[matchKey] == match[matchKey];
      });
    }
    return true;
  });
...
var _months = ["janvier", "février", "mars", "avril", "mai", "juin", "juillet",
               "août", "septembre", "octobre", "novembre", "décembre"];
var _weekDay = ["dimanche", "lundi", "mardi", "mercredi", "jeudi", "vendredi", "samedi"];

Chronology.prototype.formatDate = function(date, type) {
  switch (type || "iso") {

  case "day": return _weekDay[date.getDay()];
  case "shortday": return _weekDay[date.getDay()].substr(0, 3);
  case "month": return _months[date.getMonth()];
  case "day+number": return _weekDay[date.getDay()] + " " + date.getDate();
  case "date+month": return date.getDate()+" "+_months[date.getMonth()];

  default: return (new Date(date)).toISOString().substr(0,10);
  }
};
  ...
  if (!this.sealedEvents) {
    ...
    for (var i = 0; i < this.events.length; i++) {
      ...
      try {
        ...
        this.warn("invalid date: "+event.date+", event = "+JSON.stringify(event));
      }
      if (dateStr) {
